...
class ExternalLinks extends SpecialPage {
	...
	function execute( $query ) {
		...
		if ( $this->getUser()->isAllowed( $wgELuserRight ) ) {
			// go ahead
	...
	function headerResponseByNumber( $respNum ) {
		switch ( $respNum ) {
			...
			case "300":
				...
				break;
			case "301":
				$response = 'Moved permanently and new target works. Might be smart to fix it as long '
					. 'as it redirects.';
				break;
			case "302":
	...
	*/
	...
	function ELdoExternalLinks() {
		...
		$showEmail = $request->getVal( 'showEmail' );
		if ( $showEmail != NULL ) {
		...
		$checkResponse = $request->getVal( 'checkResponse' );
		...
		$storeSession = $request->getVal( 'storeSession' );
		...
		$filterURL = $request->getVal( 'filterURL' );

		$filterURLnot = $request->getVal( 'filterURLnot' );
		...
		$lastValidation = preg_replace( '/(\.|,).*/', '', $request->getVal( 'lastValidation' ) );

		$top = $out->addWikiMsg( 'el-intro' );
		if ( $wgELvalidationMode == NULL ) {
			$top .= $out->addWikiMsg( 'el-intro-alternative' );
		}
		$top .= '<fieldset><legend>' . $this->msg( 'el-filterButton' ) . '</legend>
						 <form name="ELform" id="ELform" style="display:inline" enctype="multipart/form-data" method="GET" action="' . $wgScriptPath . '/index.php/' . $title . '">

							 <input type="hidden" name="do" value="filter" />

							 <table><tr>

							 <td align="right"><label for="filterURL">' . $this->msg( 'el-filterURL' ) . '</label></td>
							 <td><input type="text" name="filterURL" id="filterURL" size="40" value="'.$filterURL.'">&nbsp;
							 <label for="filterURLnot">' . $this->msg( 'el-filterNot' ) . '</label><!--
							 ...
							 --><label for="showEmail">' . $this->msg( 'el-showEmail' ) . '</label>
							 </td>
		...
		if ( $wgELvalidationMode != NULL ) {
			$top .= '
							 ...
							 --><label for="checkResponse">' . $this->msg( 'el-checkResponse' ) . '</label></td>
							';
		}
		if ( $wgELenableSessionStoring != NULL ) {
			$top .= '
							 ...
							 --><label for="storeSession">' . $this->msg( 'el-storeSession' ) . '</label></td>
							 </tr><tr>
							 <td>' . $this->msg( 'el-lastValidationOfURL' ) . '</td>
							 <td><input type="text" name="lastValidation" id="lastValidation" size="3" value="'.$lastValidation.'">
							 ' . $this->msg( 'el-lastValidationDays' ) . '&nbsp; (' . $this->msg( 'el-lastValidationInfo' ) . ')
							 </td>
							';
		}
		if ( is_numeric( $pageID ) ) { //if pageID in URL parameter (so it can be removed for new query), but independent from $wgELtoolboxLink
			$top .= '</tr><tr>
							 <td align="right">' . $this->msg( 'el-pageID' ) . ':</td>
							 <td><input type="text" name="pageID" id="pageID" size="7" value="'.$pageID.'"></td>
							 ';
		}
		$top .= '</tr><tr><td></td><td><input type="submit" class="ELsubmitButton" style="margin-top:8px" value="' . $this->msg( 'el-submit' ) . '"></td>
						 </tr></table>
						 </form></fieldset>';
		$out->addHTML( $top );
		...
		$navi = '';
		$naviReturn = '';
		$text = $this->msg( 'search-nonefound', '' ); //MW default msg
		// if results
		if ( $numThis > 0 ) {
			...
			// return pagination
			$params = '&do=filter&filterURL=' . $filterURL . '&filterURLnot=' . $filterURLnot . '&showEmail='
				. $showEmail . '&checkResponse=' . $checkResponse . '&lastValidation=' . $lastValidation .
				'&storeSession=' . $storeSession . '&pageID=' . $pageID;
			$offsetNavi = ($offset - $limit);
			$naviInfo = '(' . ($offset + 1) . '&minus;' . ($offset + $numThis) . ' ' . $this->msg( 'el-nav-outOf' ) .
				' <strong>' . $numAll . '</strong>)';
			// Prev/First
			if ( $offset > 0 ) {
				$navi .= '(<a href="' . $_SERVER['PHP_SELF'] . '?offset=' . ($offsetNavi) . '' . $params . '">' .
					$this->msg( 'el-nav-prev' ) . ' ' . $limit . '</a>';
				$first = '<a href="' . $_SERVER['PHP_SELF'] . '?offset=0' . $params . '">' .
					$this->msg( 'el-nav-first' ) . '</a>';
			} else {
				$navi .= '(' . $this->msg( 'el-nav-prev' ) . ' ' . $limit;
				$first = $this->msg( 'el-nav-first' );
			}
			$navi .= ') (';
			// Next/Last
			if ( ($offset + $numThis) < $numAll ) {
				$navi .= '<a href="' . $_SERVER['PHP_SELF'] . '?offset=' . ($offset + $limit) . '' . $params .
					'">' . $this->msg( 'el-nav-next' ) . ' ' . $limit . '</a>';
				$last = '<a href="' . $_SERVER['PHP_SELF'] . '?offset=' . ($numAll - $limit) . '' . $params .
					'">' . $this->msg( 'el-nav-last' ) . '</a>';
			} else {
				$navi .= $this->msg( 'el-nav-next' ) . ' ' . $limit;
				$last = $this->msg( 'el-nav-last' );
			}
			$navi .= ')&nbsp; ';
			$navi .= $naviInfo;
			$navi .= '&nbsp; (' . $first . ' | ' . $last . ')';

			$naviReturn = $out->addHTML( '<p class="ELnavi">' );
			...
			$naviReturn .= $out->addHTML( '</p>' );

			$out->addHTML( $naviReturn );

			// return results table
			$text = '<table id="ExternalLinks" width="100%" class="' . $wgELtableClass . '">';
			$text .= '<tr><th>';
			$text .= $this->msg( 'el-th-page' );
			$text .= '</th><th>';
			$text .= $this->msg( 'el-th-url' );
			$text .= '</th>';
			if ( $checkResponse != NULL && $wgELvalidationMode != NULL ) {
				$text .= '<th>' . $this->msg( 'el-serverResponse' ) . '</th>';
			}
			$text .= '</tr>';
			foreach ( $res as $row ) {
				...
				$text .= '<tr>';
				$text .= '<td class="ELtablePage plainlinks ' . $wgELpageLinkClass . '" title="' .
					$this->msg( 'edit' ) . '">[{{fullurl:' . $title .
					'|action=edit&preview=yes&minor=1&summary=' .
					urlencode( $this->msg( 'el-edit-summary' ) ) . '#toolbar}} ' . $title . ']</td>';
				...
				if( $checkResponse != NULL && $wgELvalidationMode != NULL ) {
					...
					switch ( $wgELvalidationMode ) {
						...
						case "cURL":
							...
							if ( preg_match( '/^https:\/\//i', $link ) ) {
								...
								$respType = preg_replace( array( '/; charset=.+/', '/\//' ), array( '', ' / ' ),
					...
					if ( $respNum != NULL ) {
						if ( $respNum == 300 || $respNum == 302 ) {
							$respClass = 'ELrespNotice';
							$respBG = '#fffddf';
						}
						if ( $respNum <= 200 || $respNum == 301 ) {
							$respClass = 'ELrespOK';
							$respBG = '#e8ffe8';
						}
						if ( $respNum == "Domain Not Found?" || $respNum >= 303 ) {
						...
						$text .= '<td class="ELresp ' . $respClass . '" style="background-color:' .
							$respBG . ' !important">';
						if ( $respType ) {
							$text .= '<span class="ELrespType small grey" '
								. 'style="float:right; padding-left:8px">' . $respType . '</span>';
						}
	...
	/**
	 * Set group name for this special page
	 *
	 * @return string
	 */
